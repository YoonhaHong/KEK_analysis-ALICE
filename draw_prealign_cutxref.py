...
def extract_x_minus_y(th2, y_min, y_max, output_name):
    """
    ...
    # x축의 bin 찾기
    y_min_bin = th2.GetYaxis().FindBin(y_min)
    y_max_bin = th2.GetYaxis().FindBin(y_max)
    ...
    for x_bin in range(1, x_bins + 1):
        for y_bin in range(y_min_bin, y_max_bin):
        ...
        if not hist:
            ...
            if("correlationX_2D" in hist_path):
                for i in range( len(xrange)-1 ):
                    ...
                    latex1.DrawLatex(0.2, 0.85, f"{XMIN} < x_{{ref}} < {XMAX}")  # 텍스트 1
