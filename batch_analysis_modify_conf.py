    ...
    for region, group in tqdm(regions, ascii=True, desc="Processing geometries"):
        ...
        for _, row in tqdm(group.iterrows(), total=len(group), ascii=True, desc=f"Processing {region}", position=0):
            ...
            if "align" in description:
                print( "******************************")
                print( f"STARTING mask, prealign, align of {region} : using {runno}" )
                print( "******************************\n")
                ...
                prealigned_conf =   run_corry(run = run, stage = "prealign",    det_file_dir = det_file_dir, detectors_file = masked_conf)
            ...
            run_corry(run = run, stage = "analyse", det_file_dir = det_file_dir, nevents=200000,     detectors_file = aligned_conf, output_dir=output_dir)
