...
def plot_QA_results_12ALPIDE(corry_input,outputdir,name):
    ...
    for plane in range(12):
        for t in range(6):
            ...
            if t == 0:
                #clustermap = analyse_root.Get(f"ClusteringSpatial/ALPIDE_plane_{plane}/clusterPositionLocal")     
                #clustermap.Rebin2D(16, 16)
                #clustermap.Draw("COLZ")
                print(" ")
            elif t == 1:
                hitmap = analyse_root.Get(f"EventLoaderEUDAQ2/ALPIDE_plane_{plane}/hitmap")
